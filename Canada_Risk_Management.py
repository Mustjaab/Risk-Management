...
@app.cell
def __(pd):
    Risk_Arrangement = pd.read_csv('Risk_Arrangement.csv')
...
def __(Risk_query_2019, Risk_query_2021, db):
    Canada_Risk_Landscape_2019 = db.execute(Risk_query_2019).fetchdf()
    Canada_Risk_Landscape_2021 = db.execute(Risk_query_2021).fetchdf()
...
@app.cell
def __(mo):
    mo.md("<caption> Exploratory panel of pvalues from proportionality tests </caption>").style({'font-size':'16px'})
    return


@app.cell
def __(pd):
    Cyber_Incidents = pd.read_csv('Incidents.csv')
...
def __(Incident_query_2019, Incident_query_2021, db):
    Canada_Incident_Landscape_2019 = db.execute(Incident_query_2019).fetchdf()
    Canada_Incident_Landscape_2021 = db.execute(Incident_query_2021).fetchdf()
